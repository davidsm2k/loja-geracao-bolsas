	...
	public static void main(String[] args) {
		...
		do {
			...
			for (Bolsa prod : estoque) {
				...
				if (achou == true) {
					...
					System.out.println("(Não é possivel comprar a mesma bolsa mais de uma vez)");
					achou = false;
					break;
		...
		if(tipoPagamento == '1') {
			...
			if(parcelas == '1') {
				...
				System.out.println("\nPAGAMENTO PARCELADO: ");
				System.out.println("\nValor a do produto: 1 parcela de R$ " + Math.floor(totalPagamento)+ " (Com 10% Juros)");
			}
			else {
				MostrarHistorico();
				totalPagamento = (totalPagamento * 1.20) / 2;
				System.out.println("\nPAGAMENTO PARCELADO: ");
				System.out.println("Valor a ser pago será 2 parcelas de R$ " + Math.floor(totalPagamento) + " (Com 20% Juros)");
			}
		}
	}
